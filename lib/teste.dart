...
class CustomAudioSource extends StreamAudioSource {
  ...
  CustomAudioSource(
    ...
    cacheFile = cacheFile != null
      ...
      : _getCacheFile(uri, fileExtension: fileExtension)
  {
    // _init().then((_) => clearCache());
    _init();
  }
  ...
  static Future<File> _getCacheFile(final Uri uri, {
    String? fileExtension
  }) async {
    final dirPath = (await _getCacheDir()).path;

    final ext = fileExtension ?? p.extension(uri.path, 10);

    final fullPath = p.joinAll([
      dirPath,
      'remote',
      sha256.convert(
